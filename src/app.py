...
# Factory function that creates the auth blueprint with the auth service injected.
from src.presentation.routes.auth_routes import create_auth_routes

# Pre-built blueprints for catalogue and order routes. These have no service dependencies yet.
...
def create_app():
    ...
    # Registers the catalogue routes with Flask so the app knows to handle
    # incoming requests to /catalogue.
    app.register_blueprint(catalogue)

    # Registers the order routes with Flask so the app knows to handle
    # incoming requests to /cart.
