...
# so we replace it with a fake version that always returns the BOOKS list above.
# This is called a "fake" or "stub": a simplified stand-in for a real object
# that behaves the same way the route expects, but does nothing complicated.
...
# Any test that lists a fixture name as a parameter will automatically receive
# the value that fixture returns.
...
def get_flashes(client):
    ...
    # The session is a small dictionary that Flask keeps on the browser side,
    # signed so it cannot be tampered with.
    # Rather than following the redirect and parsing HTML to find flash messages,
    # we read them directly from the session here. This is simpler and faster.
...
# A POST request means the browser is sending data to the server, in this case
# a form submission containing the book id and quantity.
# Valid adds should flash a success message and update the cart stored in the session.
# Invalid input such as zero quantity, out-of-stock books, or unknown book ids
# should not modify the cart or show a success message.
...
def test_cart_add_valid_flashes_success(client):
    # Adding a book with a valid quantity should confirm the action to the user
...
def test_cart_add_invalid_book_id_no_flash(client):
    # A book id that does not exist in the catalogue should be silently rejected.
    # There is nothing useful to tell the user in this case as it should not
    # happen through normal use of the site.
...
def test_cart_add_negative_quantity_no_success_flash(client):
    # A negative quantity submitted via the form should be treated the same
    # as zero and not add anything to the cart. This guards against a user
    # or a browser manually sending a negative number in the form data.
...
# The cart page lets users change the quantity of a book already in their cart.
# Changing to a new quantity should flash success.
# Submitting the same quantity that is already in the cart should flash a warning
# so the user knows their change had no effect.
...
def test_cart_update_quantity_unchanged_flashes_warning(client):
    # If the user submits a quantity that matches what is already in the cart,
    # warn them so they know nothing changed.
    # session_transaction() lets us directly set session data before making a request,
    # which is how we put a book in the cart before testing the update.
    with client.session_transaction() as sess:
        sess['cart'] = {'1': 2}
...
def test_cart_update_quantity_changed_flashes_success(client):
    # Changing to a different valid quantity should confirm the update to the user.
    with client.session_transaction() as sess:
        sess['cart'] = {'1': 2}
...
def test_cart_update_quantity_changed_message_contains_title(client):
    # The success message should name the book so the user knows which item
    # was updated.
    with client.session_transaction() as sess:
        sess['cart'] = {'1': 2}
    client.post('/cart/update', data={'book_id': '1', 'quantity': '3'})
...
def test_cart_update_zero_quantity_removes_item(client):
    # Setting quantity to 0 is how a user removes an item via the update form.
    # The book should no longer appear in the cart after this.
    with client.session_transaction() as sess:
        sess['cart'] = {'1': 2}
...
def test_cart_update_book_not_in_cart_no_flash(client):
    # If the book being updated is not in the cart at all, the route should
    # silently redirect without showing any flash message.
    with client.session_transaction() as sess:
        sess['cart'] = {}
    client.post('/cart/update', data={'book_id': '1', 'quantity': '2'})
    flashes = get_flashes(client)
...
def test_cart_update_capped_at_stock(client):
    # Updating to a quantity higher than available stock should cap at the
    # stock level, the same way cart_add does. Book id 1 has 5 in stock,
    # so updating to 100 should result in 5 being stored.
    with client.session_transaction() as sess:
        sess['cart'] = {'1': 2}
...
# The remove button on the cart page sends a POST request with just the book id.
# Removing an item should flash a success message that names the book.
# Attempting to remove a book that is not in the cart should produce no flash.
...
def test_cart_remove_item_no_longer_in_cart(client):
    # After a successful remove the book should not appear in the session cart.
    with client.session_transaction() as sess:
        sess['cart'] = {'1': 2}
    client.post('/cart/remove', data={'book_id': '1'})
    with client.session_transaction() as sess:
...
def test_cart_remove_leaves_other_books_untouched(client):
    ...
    # only remove the requested book. The other books should still be there.
    with client.session_transaction() as sess:
        sess['cart'] = {'1': 2, '2': 1}
    client.post('/cart/remove', data={'book_id': '1'})
    with client.session_transaction() as sess:
...
# Clearing a non-empty cart should flash a success message that tells the user
...
def test_cart_clear_empties_cart(client):
    # After clearing, the cart in the session should be an empty dictionary.
    with client.session_transaction() as sess:
        sess['cart'] = {'1': 2}
    client.post('/cart/clear')
    with client.session_transaction() as sess:
...
def test_cart_clear_empty_cart_no_flash(client):
    # If the cart is already empty there is nothing to report, so no flash
...
def test_cart_clear_multiple_different_books(client):
    # Clearing a cart with multiple different books should remove all of them,
    # not just the first one. This test uses two different book ids to confirm.
    with client.session_transaction() as sess:
        sess['cart'] = {'1': 2, '2': 1}
    client.post('/cart/clear')
    with client.session_transaction() as sess:
...
# a URL, so we test it by loading the cart page and checking what comes back.

def test_cart_page_drops_book_removed_from_catalogue(client):
    # If a book is in the session cart but has since been removed from the
    # catalogue, the cart page should silently drop it rather than crashing.
    # This simulates a book being deleted by an admin while a customer has
    # it sitting in their cart.
    # Book id 99 does not exist in the BOOKS list used by FakeCatalogueService.
    with client.session_transaction() as sess:
        sess['cart'] = {'99': 1}
